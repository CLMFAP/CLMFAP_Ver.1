...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, n_win=7, qk_dim=None, qk_scale=None,
         ...
         print(f"dim: {dim}, qk_dim: {self.qk_dim}, num_heads: {num_heads}, self.dim= {self.dim}, self.n_win= {self.n_win}")
         assert self.qk_dim % num_heads == 0 and self.dim % num_heads==0, 'qk_dim维度必须能够整除注意力头数，否则无法形成完整多头q,v'
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        # print(f"Input bi-attention size: {x.shape}")
        # print(x)
        ...
        # print(f"Pading Input bi-attention size: {x.shape}")
        ...
        q, kv = self.qkv(x)
        # print(f"Bi attention q size:{q.shape}")
        # print(f"Bi-attention kv size: {kv.shape}")
        ...
        # print(f"Bi-attention q_pix size: {q_pix.shape}")
        # print(f"Bi-attention kv_pix size: {kv_pix.shape}")

        q_win, k_win = q.mean([2, 3]), kv[..., 0:self.qk_dim].mean([2, 3]) # window-wise qk, (n, p^2, c_qk), (n, p^2, c_qk)
        # print(f"Bi-attention q_win size: {q_win.shape}")
        # print(f"Bi-attention k_win size: {k_win.shape}")

...
class gitDataset(BaseDataset):
    def __init__(
        self, data_path, config, split, encoder=False, task=None, transform=None
    ):
        ...
        self.label_tokenizer = T5Tokenizer.from_pretrained(
        ...
        self.processor = Blip2Processor(
            self.processor.current_processor, self.label_tokenizer
        )
        # graph_feature
        graph2d_featurizer_config = {"name": "ogb"}
        self.task_list = task
    ...
    def __getitem__(self, i):
        ...
        if "caption" in self.outputs_modal:
            ...
            caption_labels = self.processor(text=caption, return_tensors="pt")[
                "input_ids"
            ]

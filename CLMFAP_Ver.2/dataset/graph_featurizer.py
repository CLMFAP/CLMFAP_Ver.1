...
class GraphFeaturizer(BaseFeaturizer):
        ...
        for atom in mol.GetAtoms():
            if self.config["name"] == "ogb":
                atom_feature = [
                    ...
                    self.allowable_features["possible_is_aromatic_list"].index(
                        atom.GetIsAromatic()
                    ),
                    self.allowable_features["possible_is_in_ring_list"].index(
                        atom.IsInRing()
                    ),
                ]
            else:
            ...
            for bond in mol.GetBonds():
                ...
                if self.config["name"] == "ogb":
                    edge_feature = [
                        ...
                        self.allowable_features["possible_bond_stereo_list"].index(
                            bond.GetStereo()
                        ),
                        self.allowable_features["possible_is_conjugated_list"].index(
                            bond.GetIsConjugated()
                        ),
                    ]
                else:
                    edge_feature = [
                        self.allowable_features["possible_bond_type_list"].index(
                            bond.GetBondType()
                        ),
    ...
    def generate_dicts(self):
        atomic_num_dict = {
            atom_num: i
            for i, atom_num in enumerate(
                self.allowable_features["possible_atomic_num_list"]
            )
        }
        bond_type_dict = {
            bond_type: i
            for i, bond_type in enumerate(
                self.allowable_features["possible_bond_type_list"]
            )
        }

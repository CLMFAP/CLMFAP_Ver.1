# !/bin/bash
# SBATCH --nodes=1
# SBATCH --gpus-per-node=v100l:1
# SBATCH --ntasks-per-node=1
# SBATCH --mem=80000M
# SBATCH --time=48:00:00
# SBATCH --account=def-hup-ab

nvidia-smi
echo 'Emm, Grey Cat!'

# module load StdEnv/2020 gcc/9.3.0 cuda/11.7 arrow/9.0.0 python/3.9
# source /home/user54/projects/def-hup-ab/user54/smiles_transformer_env/bin/activate

#cd ../../apex
#export TORCH_CUDA_ARCH_LIST="7.0"
#pip install -v --disable-pip-version-check --no-cache-dir --no-build-isolation --global-option="--cpp_ext" --global-option="--cuda_ext" ./

python finetune_contrastive_learning_regression.py \
        --smiles_graph_weight 0.4 \
        --smiles_fp_weight 0.4 \
        --graph_fp_weight 0.2 \
        --graph_model 'bi_graphormer_mpnn' \
        --model_path '/home/user/workspace/MY_MODEL/result/finetune_result/data/clintox/bi_graphormer_mpnn/checkpoint.ckpt' \
        --dataset_name clintox \
        --test True
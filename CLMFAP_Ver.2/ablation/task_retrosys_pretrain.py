...
class CombinedMoleculeDataset(Dataset):
    def __init__(self, data):
        ...
        self.target_text_encoder = Encoder(1000, mlm_probability = 0.0)

        print("Loading data...")
        self.target_encodings = self.target_text_encoder.process(self.smiles_list)
...
def train(model, cl_model, data_loader, num_epochs, device, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(data_loader):
            ...
            molecule_emb = cl_model(
                input_ids, fingerprints, graphs, graphormer_datas
            )

            target_emb = cl_model(
                target_input_ids, fingerprints, graphs, graphormer_datas
            )

            # plm_input = {"net_input0": {"src_tokens": src_tokens}}  # Assuming this structure for PLM input
            plm_input = molecule_emb
            # Forward pass
            output = model(
                src_tokens=molecule_emb,
                src_lengths=768,
                prev_output_tokens=molecule_emb,
                plm_input=plm_input,
            ...
            logits, _ = output
            logits = logits.view(-1, logits.size(-1))  # Flatten logits for CrossEntropyLoss
            # target = target_tokens.view(-1)  # Flatten targets
            # print(logits)
            # print(target_emb)

            cos_sim = nn.functional.cosine_similarity(logits, target_emb, dim=-1)
            loss = 1 - cos_sim
            loss = loss.mean()
        ...
        if (epoch+1) % 4 == 0:
            torch.save(
                model.state_dict(),
                f"result_retrosys/{graph_model}/checkpoint_{epoch}.ckpt",
            )

...
def train(model, cl_model, data_loader, num_epochs, device, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(data_loader):
            ...
            target_tokens = batch["target_input_ids"].to(device)
            padding_size = 768 - target_tokens.size(1)
            target_tokens = nn.functional.pad(target_tokens, (0, padding_size))
            ...
            cos_sim = nn.functional.cosine_similarity(logits, target_tokens, dim =-1)
            loss = 1 - cos_sim
            loss = loss.mean()
        ...
        if (epoch+1) % 4 == 0:
            torch.save(
                model.state_dict(),
                f"result_retrosys/{graph_model}/finetune/checkpoint_{epoch}.ckpt",
            )
...
model.load_state_dict(
    torch.load(f"result_retrosys/{graph_model}/checkpoint_19.ckpt"), strict=False
)

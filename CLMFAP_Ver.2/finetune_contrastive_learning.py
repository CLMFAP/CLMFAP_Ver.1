...
class MolecularEmbeddingModel(nn.Module):
    ...
    class Net(nn.Module):
        ...
        def __init__(self, smiles_embed_dim, num_classes, dims=dims, dropout=0.4):
            super().__init__()
...
def train(
    ...
    # loss_fun = nn.BCELoss()
    print(f"{result_folder}/loss.txt")
    ...
    for epoch in range(epochs):
        ...
        if epoch != 0 and epoch > 10 and epoch % 4 == 0:
            torch.save(
                model.state_dict(),
...
if main_args.test:
    ...
    for dataset_name in ['MIC',"MIC_1", "MIC_2","bace","bace_1","bace_2",'bbbp', "bbbp_1", "bbbp_2", 'PAMPA',"PAMPA_1", "PAMPA_2", 'Bioavailability',"Bioavailability_1", "Bioavailability_2",'antibiotic_0', "antibiotic_1", "antibiotic_2"]:
    # for dataset_name in ['bbbp', "bbbp_1", "bbbp_2"]:
    # for dataset_name in ["bbbp_2"]:
    # for dataset_name in ['MIC', 'bbbp', 'PAMPA', 'Bioavailability', 'antibiotic_0', 'bace_0']:
    # for dataset_name in ['Ecoli_Activity_0', 'Ecoli_Activity_1','Ecoli_Activity_2', 'Ecoli_BW25113_Activity_0','Ecoli_BW25113_Activity_1','Ecoli_BW25113_Activity_2']:
    # for dataset_name in ['Ecoli_Activity_0', 'Ecoli_BW25113_Activity_0']:
    ...
    # for dataset_name in ["bbbp_1", "bbbp_2", "PAMPA_1", "PAMPA_2", "antibiotic_0", "antibiotic_1", "antibiotic_2"]:
        ...
        # for graph_model in ["bi_graphormer_mpnn","no_graphormer_mpnn","original_graphormer_mpnn","original_graphormer_no_mpnn"]:
            ...
            #     model_path = f"/home/user/workspace/MY_MODEL/result/finetune_result/{dataset_name}/{graph_model}/checkpoint_32.ckpt"
            #     model_path = f"/home/user/workspace/MY_MODEL/result/finetune_result/{dataset_name}/{graph_model}_fp_radius_{fp_radius}/checkpoint_24.ckpt"
        ...
        # for save_model_folder in ["nt_xent_3_mean_remove_sub","nt_xent_3_random_remove_sub","nt_xent_3_sum_remove_sub","bce_with_logits_remove_sub","bce_with_logits_remove_sub_batch_size_32"]:
        for save_model_folder in ["checkpoint_4"]:
        # for save_model_folder in ["checkpoint_1","checkpoint_2","checkpoint_3","checkpoint_4","checkpoint_5","checkpoint_6","checkpoint_7","checkpoint_8"]:
        ...
        # for graph_model in ["bi_graphormer_no_mpnn"]:
            ...
            measure_name = config['data'][dataset_name]['measure_name']
            num_classes = config['data'][dataset_name]['num_classes']
            result_folder = f"{config['result']['test_path']}/loss_bce_diff_epoch/{dataset_name}/{graph_model}_{save_model_folder}/"
            ...
            # model_path = f"/home/user/workspace/MY_MODEL/result/finetune_result/{dataset_name}/{graph_model}_save/checkpoint_32.ckpt"
            # model_path = f"{config['result']['finetune_path']}/{dataset_name}/{graph_model}_{save_model_folder}/checkpoint_24.ckpt"
            model_path = f"{config['result']['finetune_path']}/sugi/{dataset_name}/{graph_model}/checkpoint_8.ckpt"
            model_path = f"{config['result']['finetune_path']}/loss_bce_diff_epoch_with_pampa/{dataset_name}/{graph_model}_{save_model_folder}/checkpoint_24.ckpt"
            if  "MIC" in dataset_name:
                model_path = f"{config['result']['finetune_path']}/loss_bce_diff_epoch_with_pampa/{dataset_name}/{graph_model}_{save_model_folder}/checkpoint_32.ckpt"
            # model_path = main_args.model_path
            print(f"Model path: {model_path}")
            model.load_state_dict(
                torch.load(model_path), strict=False
    ...
    # for dataset_name in ["antibiotic_0", "antibiotic_1", "antibiotic_2"]:
        ...
        # for graph_model in ["bi_graphormer_no_mpnn"]:
        # for save_model_folder in ["nt_xent_3_mean_remove_sub","nt_xent_3_random_remove_sub","nt_xent_3_sum_remove_sub","bce_with_logits_remove_sub","bce_with_logits_remove_sub_batch_size_32"]:
        # for save_model_folder in ["checkpoint_1","checkpoint_2","checkpoint_3","checkpoint_4","checkpoint_5","checkpoint_6"]:
        for save_model_folder in ["checkpoint_4"]:
        # for save_model_folder in ["nt_xent_3_mean_remove_sub_1","bce_with_logits_remove_sub_1","bce_with_logits_remove_sub_batch_size_8_1","bce_with_logits_remove_sub_batch_size_32_1"]:
            graph_model = "bi_graphormer_no_mpnn"
            print(f"Model name: {graph_model}")
            ...
            result_folder = f"{config['result']['finetune_path']}/{dataset_name}/{graph_model}_save/"
            result_folder = f"{config['result']['finetune_path']}/loss_bce_diff_epoch_bbbp/{dataset_name}/{graph_model}_{save_model_folder}/"
            ensure_folder_exists(result_folder)
            # model_path = main_args.model_path
            # result_folder = f"{config['result']['finetune_path']}/{dataset_name}/{graph_model}/"
            # ensure_folder_exists(result_folder)
            ...
            model_path = f"result/pretrain_result_{graph_model}_bce_with_logits_remove_sub_2/{save_model_folder}.ckpt"
            # model_path = f"{config['result']['finetune_path']}/loss_bce_diff_epoch/{dataset_name}/{graph_model}_{save_model_folder}/checkpoint_24.ckpt"
            ...
            # model_path = f"/home/user/workspace/MY_MODEL/result/pretrain_result_bi_graphormer_no_mpnn_wholechembl_442/checkpoint_3.ckpt"
            train_data = pd.read_csv(
                f"{data_path}/train.csv",
                sep=",",
                header=0
    ...
    data_path = config['data'][main_args.dataset_name]['data_root']
    measure_name = config['data'][main_args.dataset_name]['measure_name']
    num_classes = config['data'][main_args.dataset_name]['num_classes']
    result_folder = f"{config['result']['finetune_path']}/{main_args.save_model_folder}/{main_args.dataset_name}/"
    ensure_folder_exists(result_folder)
    model_path = main_args.model_path

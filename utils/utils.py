...
def print_model_info(model, level=0, prefix=""):
    ...
    for name, module in model.named_children():
        ...
        print(
            f"{prefix}Module: {name} | Total parameters: {total_params_module} | Trainable parameters: {trainable_params_module}"
        )
    ...
    if prefix == "":
        print(
            f"Total parameters: {total_params} | Trainable parameters: {trainable_params} | Trainable ratio: {trainable_params / total_params:.2%}"
        )
    ...
    def __init__(self, mode="higher", patience=10, filename=None, metric=None):
        if filename is None:
            ...
            filename = os.path.join(
                folder,
                "early_stop_{}_{:02d}-{:02d}-{:02d}.pth".format(
                    dt.date(), dt.hour, dt.minute, dt.second
                ),
            )
        else:
            dt = datetime.datetime.now()
            filename = filename + "_{}_{:02d}-{:02d}-{:02d}.pth".format(
                dt.date(), dt.hour, dt.minute, dt.second
            )
